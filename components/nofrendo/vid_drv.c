...
/* ESP-IDF 移植(C3):BLE 主机 + codec DMA + 内核 ROM 拷贝已占掉大半堆,
 * 运行时 malloc 整块 8bpp 帧缓冲(256x240=61KB)实测失败 → primary_buffer 为 NULL,
 * 首帧 ppu_renderscanline 解引用即崩(Load access fault,MTVAL=0x14)。
 * 改为链接期静态缓冲:bmp_createhw 不拥有数据,bmp_destroy 也不会释放它。 */
static uint8 s_renderbuf[256 * 240];   /* NES_SCREEN_WIDTH x NES_SCREEN_HEIGHT,8bpp */
...
//      bmp_destroy(&back_buffer);
   ...
   if (width * height > (int) sizeof(s_renderbuf))
      return -1;

   primary_buffer = bmp_createhw(s_renderbuf, width, height, width); /* no overdraw */
   if (NULL == primary_buffer)
      return -1;
