...
static struct
{
   int counter, latch;
   bool enabled, reset;
   uint32 fired;   /* 诊断:IRQ 触发累计 */
   uint32 arms;    /* 诊断:C001/E001 写入累计 */
} irq;
...
//         irq.counter = irq.latch;
   ...
   case 0xC001:
      ...
      irq.counter = irq.latch;
      if (irq.arms < 20)
         log_printf("[map4] C001 重载 latch=%d", irq.latch);
      irq.arms++;
      break;
...
//         irq.counter = irq.latch;
   ...
   case 0xE001:
      irq.enabled = true;
      if (irq.arms < 40)
         log_printf("[map4] E001 开中断 (第%d次)", irq.arms);
      irq.arms++;
//      if (irq.reset)
//         irq.counter = irq.latch;
      break;
   ...
   if (ppu_enabled())
   {
      if (irq.counter >= 0)
      {
         ...
         if (irq.counter < 0)
         {
            if (irq.enabled)
            {
               /* 诊断:标题卡死排查——看 IRQ 是否还在触发(最多 3s 一条) */
               irq.fired++;
               {
                  extern volatile int nofrendo_ticks;
                  static int s_last_diag_tick;
                  if (nofrendo_ticks - s_last_diag_tick >= 180)   /* ~3s */
                  {
                     s_last_diag_tick = nofrendo_ticks;
                     log_printf("[map4] MMC3 IRQ %lu 次 (latch=%d)",
                                (unsigned long)irq.fired, irq.latch);
                  }
               }
               irq.reset = true;
               nes_irq();
