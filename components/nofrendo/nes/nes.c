...
/* main emulation loop */
/* ESP-IDF 补丁: osd_yield 由移植层实现(vTaskDelay),防忙等饿死低优先级任务 */
extern void osd_yield(void);
/* ESP-IDF 补丁: 跳帧分支产音(防 I2S 断流破音),由移植层实现 */
extern void osd_audio_frame(void);

void nes_emulate(void)
{
   int last_ticks, frames_to_render;
   static int s_consec_skip = 0;   /* ESP-IDF 补丁:连跳帧计数(防画面冻结) */
   ...
   while (false == nes.poweroff)
   {
      ...
      /* ESP-IDF 补丁:积压帧数雪崩上限。某帧一旦慢于 1 tick(标题等重负载),
       * tick 持续积累而每轮只消耗 1,f2r 永远回不到 1 -> 画帧分支永久饥饿,
       * 表现为画面冻结但跳帧分支继续产音(音乐在响)。上限 4:最差 3跳1画。 */
      if (frames_to_render > 4)
         frames_to_render = 4;
      ...
      {
         frames_to_render--;
         /* ESP-IDF 补丁:防雪崩——最多连跳 3 帧,第 4 帧强制画图。
          * 否则持续过载时 f2r 永远 >1,画帧分支永久饥饿,
          * 画面冻结而跳帧分支继续产音(音乐在响,实测冒险岛3标题)。 */
         if (++s_consec_skip >= 4)
         {
            s_consec_skip = 0;
         ...
         else
         {
            ...
            system_video(false);
            osd_audio_frame();   /* 跳帧不推屏,但一帧音频不能少 */
         }
      }
