...
\
   ...
   { \
      apu.rectangle[ch].accum += (apu.rectangle[ch].freq + 1) << 8; \
...
static int32 apu_triangle(void)
{
   ...
   {
      apu.triangle.accum += apu.triangle.freq << 8;
...
static int32 apu_noise(void)
{
   ...
   {
      apu.noise.accum += apu.noise.freq << 8;
...
{  
   ...
   case APU_WRC3:
      ...
      ** the other triregs
      */
      apu.triangle.write_latency = (int) ((228 << 8) / apu.cycle_rate);
...
void apu_setparams(double base_freq, int sample_rate, int refresh_rate, int sample_bits)
{
   ...
   apu.cycle_rate = (int32) ((apu.base_freq * 256.0) / sample_rate);   /* 优化C7: Q8 定点 */
