...
#ifndef __cplusplus
#include <stdbool.h>
/* ESP-IDF 补丁:原来这里 typedef enum {...} bool; —— GNU ABI 下 enum 是 4 字节。
 * 一旦某个编译单元(如引入 stdbool.h 的 nes.c)把 bool 变成 1 字节 _Bool,
 * 跨单元共享的 apu_t 等结构体就会布局分裂(实测 process 偏移 352 vs 280),
 * 函数指针全部错位 -> 跳 0xc000 崩机。统一用 stdbool 的 bool(1 字节)。 */
